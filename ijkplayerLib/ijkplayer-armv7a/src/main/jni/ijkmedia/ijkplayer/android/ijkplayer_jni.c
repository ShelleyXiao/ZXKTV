...
static int IjkMediaPlayer_getAudioTrackNum(JNIEnv *env, jobject thiz) {
	int ret = -1;

    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
    JNI_CHECK_GOTO(mp, env, "java/lang/IllegalStateException", "mpjni: getAudioTrackNum: null mp", LABEL_RETURN);
...
static void IjkMediaPlayer_switchAudioTrack(JNIEnv* env, jobject thiz, jint trackNums, jint index) {
	IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
	JNI_CHECK_GOTO(mp, env, "java/lang/IllegalStateException", "mpjni: setFrameAtTime: null mp", LABEL_RETURN);
	if(trackNums <= 0 || index <= 0) {
		goto LABEL_RETURN;
	}
	
	ijkmp_switch_audio_track(mp, trackNums, index);
...
static JNINativeMethod g_methods[] = {
	...
	{"getAudioTrackNum", "()I", (void *) IjkMediaPlayer_getAudioTrackNum},
	{"switchAudioTrack", "(II)V", (void *)IjkMediaPlayer_switchAudioTrack},
};
