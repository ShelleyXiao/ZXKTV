...
void *decodVideoT(void *data) {
    VideoDecoder *video = (VideoDecoder *) data;
    video->decodVideo();
...
void *codecFrame(void *data) {
    VideoDecoder *wlVideo = (VideoDecoder *) data;
...
void VideoDecoder::playVideo(int type) {
    codecType = type;
    if (codecType == 0) {
...
void VideoDecoder::setClock(int secds) {
    clock = secds;
}
