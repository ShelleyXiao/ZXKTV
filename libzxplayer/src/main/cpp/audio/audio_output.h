...
typedef int(*audioPlayerCallback)(void *, size_t, void *ctx);

typedef void (*pcmBufferCallBack)(SLAndroidSimpleBufferQueueItf, void *);
...
public:
    ...
    SLresult initSoundTrack(int channels, int accompanySampleRate,
    ...
    SLresult setVolume(int percent);

    SLresult setVolMute(SLboolean mute);

    SLresult setChannelMute(int mute);
...
//	audioPlayerCallback produceDataCallback;
    ...
    SLAndroidSimpleBufferQueueItf getSLQueueItf();

    pcmBufferCallBack produceDataCallback;
    void *ctx;

    int volumePercent = 100;
    SLresult volMute = false;
    int mute = 2;
...
private:
    ...
    SLresult getAudioPlayerVolumInterface();

    SLresult getAudioPlayerMuteInterface();
