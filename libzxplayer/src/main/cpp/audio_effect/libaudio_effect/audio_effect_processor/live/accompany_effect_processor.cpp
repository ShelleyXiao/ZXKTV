...
AccompanyEffectProcessor::AccompanyEffectProcessor()
        : audioEffect(NULL), changedEffect(NULL) {
...
AudioResponse *
AccompanyEffectProcessor::processAccompany(short *accompanyBuffer, int accompanyBufferSize,
                                           float position, long frameNum) {
    response->clear();
...
void AccompanyEffectProcessor::detectRebuildFilterChain() {
    if (isAudioEffectChanged) {
        ...
        if (this->changedEffect != NULL) {
            delete (this->changedEffect);
            this->changedEffect = NULL;
        }
...
void AccompanyEffectProcessor::setAudioEffect(AudioEffect *audioEffect) {
    ...
    this->changedEffect = this->audioEffect;
    this->audioEffect = audioEffect;
}
